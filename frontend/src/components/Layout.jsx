  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      <aside className={`fixed lg:fixed lg:top-0 inset-y-0 left-0 z-50 w-60 sm:w-64 max-w-[85vw] bg-primary-800 text-white shadow-xl transform transition-transform lg:translate-x-0 flex flex-col h-screen max-h-screen ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mt-auto mb-8 sm:mb-6 lg:mb-0 p-3 sm:p-4 border-t border-primary-700">
          <NavLink
            to="/change-password"
            onClick={() => setSidebarOpen(false)}
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen lg:ml-64">
        <header className="sticky top-0 shrink-0 z-30 bg-white border-b border-gray-200 px-2.5 sm:px-4 lg:px-6 py-2 flex items-center justify-between no-print shadow-sm">
        ...
        <main className="relative z-0 flex-1 min-w-0 overflow-x-hidden p-3 sm:p-4 lg:p-8">
          <Outlet />
        </main>

        <footer className="no-print border-t border-primary-500 bg-primary-700 px-3 sm:px-4 lg:px-8 py-3 text-xs sm:text-sm text-white shadow-2xl">
